...
The vocabulary of this document assumes that there are multiple "players" at your table, and only one "narrator." However, any number of players can fit into these two groups to fit the needs of any story.
...
Every character has a list of proficiencies. These are easy to keep track of on a sheet of paper. The narrator may ask a player to roll to see if their character can jump across a deep pit. That player rolls a 20 sided die (d20), then looks at their sheet. They have a list of proficiencies. They pick out all the ones that would be relevant to their character's ability to jump across a deep pit. Those proficiencies have set numeric values. The player adds those values together, then adds the result of their d20 roll. The player tells the narrator the total, and the narrator relays whether that number was high enough to meet their TN or not. Then the narrator describes the scene as the character succeeds or fails at jumping across the hypothetical pit. The player does not need to relay to the narrator which proficiencies they used unless the narrator asks for them.
...
>* The characters can only spend LP at a narratively appropriate moment in the story. For example asking a craftsman for lessons at their craft so the player can buy some proficiency in their tools, or reading a book about a proficiency that the player wants to buy.
...
> Reminder! Spyder does not assume the setting or type of story being told! Though this section seems to emphasize highly combative scenes, these rules apply to any scenario when timing is crucial. Please trust your narrator to judge how these rules do or don't get used in your story.

## Combat
Not all stories need to include combat, but for the ones that do, the combat can be very decisive in the outcome of the story. That is why there are so many combat related rules in this document. If your story may include combat, please consider the following:

Every character has a resource called "Hit Points" (HP). **HP represents the number of mortal wounds a character can receive before dying.**. If a character has 0 or fewer hit points, that character is now dead. Depending on your story, death may not be the end, so define what happens at 0 hit points before it comes up. When a character is inflicted with a mortal wound, they lose 1 hit point. Depending on your narrator's ruling, hit points may or may not be restored by various means, but they cannot surpass a character's "Maximum Hit Points" (Max HP).

A character's Max HP is how many HP they have when they've been inflicted by no wounds at all. **A character's Max Hit Points are equal to 1 + their "Base Hit Points" (Base HP) proficiency + their Skin Proficiency**.
